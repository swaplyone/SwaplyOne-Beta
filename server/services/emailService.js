...
function getEmailConfig() {
  ...
  const from = process.env.EMAIL_FROM || process.env.SMTP_FROM || '"Swaply" <founder@swaplyone.in>';
...
export async function sendOtpEmail(email, otp) {
  ...
  return sendEmail({
    to: email,
    subject: `🔑 ${otp} is your Swaply Beta Verification Code`,
    html,
    emailType: 'otp'
  });
}
...
export async function sendRegistrationSuccessEmail(email, name, betaId) {
  const html = `
    <div style="font-family: 'Plus Jakarta Sans', Arial, sans-serif; background-color: #FBF5EC; padding: 30px; border-radius: 16px; border: 3px solid #1B242A; max-width: 520px; margin: 0 auto;">
      ...
      <p style="color: #1B242A; font-size: 14px; line-height: 1.6; font-weight: 500;">Your Swaply Beta registration has been confirmed. Below is your official Beta Pass ID:</p>
  ...
  return sendEmail({
    to: email,
    subject: `🎉 Registration Confirmed! Your Swaply Beta Pass: ${betaId}`,
    html,
...
export async function sendWelcomeEmail(email, name, betaId) {
  ...
  return sendEmail({
    to: email,
    subject: `🚀 Welcome to Swaply Beta, ${name}!`,
    html,
    emailType: 'welcome'
  });
}
