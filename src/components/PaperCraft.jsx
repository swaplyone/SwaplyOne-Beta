...
// PAPER CLIP SVG
export function PaperClip({ className = "w-6 h-10 text-swaply-black drop-shadow-md", style }) {
  return (
    <svg className={className} style={style} viewBox="0 0 24 48" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round">
...
// BINDER CLIP ACCENT
export function BinderClip({ className = "w-10 h-10 text-swaply-black drop-shadow-md" }) {
  return (
...
// MASKING TAPE STRIP
export function MaskingTape({ className = "w-28 h-7 bg-amber-100/70 border border-amber-300/60 shadow-sm rotate-[-4deg] opacity-90" }) {
  return (
    <div className={`pointer-events-none z-10 backdrop-blur-[1px] ${className}`} style={{
...
export function StickyNote({ title, text, rotation = "rotate-[2deg]", color = "bg-[#FEF9C3]" }) {
  return (
    <div className={`p-4 ${color} text-swaply-black border-2 border-swaply-black/80 shadow-hard-sm ${rotation} rounded-sm relative max-w-xs font-sans`}>
      <MaskingTape className="w-20 h-5 bg-amber-100/80 border border-amber-200/80 -top-3 left-1/2 -translate-x-1/2 absolute" />
